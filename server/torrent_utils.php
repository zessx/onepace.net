<?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/packages/PHP-Torrent-Scraper-master/httptscraper.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/packages/PHP-Torrent-Scraper-master/udptscraper.php';
...
function get_torrent($path) {
    ...
    if($extension == 'torrent') {
        ...
        $now = new DateTime();
        $then = new DateTime($created);
        $interval = $now->diff($then);
        $new = $interval->days <= 7;

        return [
            'new' => $new,
            'dir' => '/torrents/' . $file_name,
            'hash' => $hash,
            'trackers' => $trackers,
            'magnet' => $magnet_url,
