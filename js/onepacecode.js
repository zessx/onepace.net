...
function generatePost(identifier, header, body, footer) {
  ...
  return $.ajax({
    url: '/server/get_episode.php', data: { crc32: identifier }, success: function (data) {
      $(selector).append(episodeString(data, header, body, footer));
    }, error: function () {
      $.ajax({
        url: '/server/get_arc.php', data: { torrent_hash: identifier }, success: function (data) {
...
function loadStreams() {
  ...
  var cookie = getCookie('last-stream');
  var url = new URL(window.location.href);
  var id = url.searchParams.get("episode");
  $.ajax({ url: "/server/get_streams.php", dataType: 'json' }).done(function (data) {
    episode_data = data;
    setArcsSelector(cookie, id);
    changeArc();
  });
}
...
var selectedArc = {};
function setArcsSelector(cookie, id) {
  ...
  $.each(episode_data['arcs'], function (i) {
    ...
    if(id > 0) {
      if(this['episodes'].filter(i => i.id === id).length > 0) {
        $('select.arcs')[0].selectedIndex = i;
        selectedArc = this;
        remembered = true;
      }
    }
    else if(typeof cookie !== 'undefined' && cookie != null && !remembered) {
      var arcid = cookie.substr(0, cookie.indexOf(','));
      if (arcid == this['id']) {
        $('select.arcs')[0].selectedIndex = i;
        selectedArc = this;
        remembered = true;
  ...
  if ($option.attr('id')) {
    ...
    if (arc) {
      ...
      $.each(episodes, function (i, episode) {
        ...
        if (i == 0) {
          $('select.episodes').html('<option id="' + episode['id'] + '">' + title + '</option>');
        } else {
          $('select.episodes').append('<option id="' + episode['id'] + '">' + title + '</option>');
        }
        ...
        else if (typeof c !== 'undefined' && c != null && !remembered) {
          var cid = c.substr(c.indexOf(',') + 1);
          if (episode['id'] == cid) {
            $('select.episodes')[0].selectedIndex = i;
            remembered = true;
          }
...
function setStream() {
  ...
  var episode = find(selectedArc['episodes'], 'id', file);
  ...
  $('img.streaming-thingy').replaceWith('<video controls poster="http://placehold.it/' + resolution + '/222/fff?text=►" style="width:100%;min-height:480px;" class="streaming-thingy" src="/streams/' + episode['crc32'] + '.mp4"></video>');
  $('video.streaming-thingy').replaceWith('<video controls poster="http://placehold.it/' + resolution + '/222/fff?text=►" style="width:100%;min-height:480px;" class="streaming-thingy" src="/streams/' + episode['crc32'] + '.mp4"></video>');
  var cvalue =  selectedArc.id + ',' + episode['id'];
